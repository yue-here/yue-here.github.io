...
3. A [vision encoder-decoder model](https://huggingface.co/docs/transformers/model_doc/vision-encoder-decoder) that converts images to the appropriate text - here, the above GPT-2 model acts as the text generator.
...
GPT-2 is based on what is known as a decoder-only model (at this point, what I've dubbed "Godwin's law but for AI" comes into effect - I direct you to the original transformers paper, <a href=https://arxiv.org/abs/1706.03762>"Attention Is All You Need" </a>). These models are good for text generation, but the base model is designed to generate text of an arbitrary length, so I had to make a couple of tweaks to get the model to generate paper-title-style sentences.
